...
class fastqExtractor:
    ...
    def extract(self, result_dict):
        ...
        set_result_value(self, result_dict, "read.pass.count", count_boolean_elements(self.dataframe_1d, "passes_filtering", True))
        set_result_value(self, result_dict, "read.fail.count", count_boolean_elements(self.dataframe_1d, "passes_filtering", False))
        total_reads = get_result_value(self, result_dict, "read.count")

        # Ratios
        set_result_value(self, result_dict, "read.pass.ratio", (get_result_value(self, result_dict, "read.pass.count") / total_reads))
        set_result_value(self, result_dict, "read.fail.ratio", (get_result_value(self, result_dict, "read.fail.count") / total_reads))
        ...
        if self.rich:
            ...
            for index, value in occupancy_channel(self.dataframe_1d).items():
                set_result_value(self, result_dict, "channel.occupancy.statistics." + index, value)
        ...
        if self.rich and self.is_barcode:
            extract_barcode_info(self, result_dict, self.barcode_selection, self.dataframe_dict, self.dataframe_1d)
    ...
    def _load_fastq_data(self):
        ...
        rst_futures = multiprocessing_submit(self._fastq_batch_reader, read_batchs, n_process=self.thread, pbar_update=self.batch_size)
        fq_df = []
