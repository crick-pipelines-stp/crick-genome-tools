...
def sorted_series_boolean_elements_divided(dataframe, column_name1: str, column_name2: str, boolean: bool, denominator: int):
    """
...
def fill_series_dict(df_dict, df):
    ...
    for read_type in ["pass", "fail"]:
        ...
        # Read length series
        df_dict[read_type + ".reads.sequence.length"] = series_cols_boolean_elements(df, "sequence_length", "passes_filtering", read_type_bool)

        # Read qscore series
        df_dict[read_type + ".reads.mean.qscore"] = series_cols_boolean_elements(df, "mean_qscore", "passes_filtering", read_type_bool)
...
def extract_barcode_info(extractor, result_dict, barcode_selection, dataframe_dict, df):
    ...
    df_base_pass_barcode = series_cols_boolean_elements(df, ["barcode_arrangement", "sequence_length"], "passes_filtering", True)

    dataframe_dict["base.pass.barcoded"] = _barcode_bases(extractor, barcode_selection, result_dict, "base.pass.barcoded", df_base_pass_barcode)

    df_base_fail_barcode = series_cols_boolean_elements(df, ["barcode_arrangement", "sequence_length"], "passes_filtering", False)

    dataframe_dict["base.fail.barcoded"] = _barcode_bases(extractor, barcode_selection, result_dict, "base.fail.barcoded", df_base_fail_barcode)

    # Get barcodes frequency by read type
    series_read_pass_barcode = series_cols_boolean_elements(df, "barcode_arrangement", "passes_filtering", True)

    dataframe_dict["read.pass.barcoded"] = _barcode_frequency(
        extractor, barcode_selection, result_dict, "read.pass.barcoded", series_read_pass_barcode
    )

    series_read_fail_barcode = series_cols_boolean_elements(df, "barcode_arrangement", "passes_filtering", False)

    dataframe_dict["read.fail.barcoded"] = _barcode_frequency(
        extractor, barcode_selection, result_dict, "read.fail.barcoded", series_read_fail_barcode
    )
    ...
    set_result_value(extractor, result_dict, "read.pass.barcoded.frequency", (read_pass_barcoded_count / total_reads) * 100)

    # Add key "read.fail.barcoded.frequency"
    set_result_value(extractor, result_dict, "read.fail.barcoded.frequency", (read_fail_barcoded_count / total_reads) * 100)

    # Replaces all rows with unused barcodes (ie not in barcode_selection) in column barcode_arrangement with the 'other' value

    df.loc[~df["barcode_arrangement"].isin(barcode_selection), "barcode_arrangement"] = "other barcodes"
    ...
    for index_barcode, barcode in enumerate(barcode_selection):
        ...
        _barcode_stats(extractor, result_dict, barcode_all_reads_df, barcode_pass_reads_df, barcode_fail_reads_df, barcode)

    # Add filtered dataframes (all info by barcode and by length or qscore) to dataframe_dict
    _barcode_selection_dataframe(dataframe_dict, df, "sequence_length", "barcode_selection_sequence_length_dataframe", "length")
    _barcode_selection_dataframe(dataframe_dict, df, "mean_qscore", "barcode_selection_sequence_phred_dataframe", "qscore")


def _barcode_selection_dataframe(dataframe_dict, df, df_column_name: str, df_key_name: str, melted_column_name: str):
    """
    ...
    filtered_df = df.filter(items=["passes_filtering", df_column_name, "barcode_arrangement"])

    # Reshape dataframe with new MultiIndex : numbered index of df length + passes filtering index and then shape data by barcode
    barcode_selection_dataframe = filtered_df.set_index([pd.RangeIndex(start=0, stop=nrows), "passes_filtering"], drop=True).pivot(
...
def _barcode_stats(
    extractor, result_dict, barcode_selected_dataframe, barcode_selected_read_pass_dataframe, barcode_selected_read_fail_dataframe, barcode_name
):
    """
    :param result_dict:
...
def _barcode_frequency(extractor, barcode_selection, result_dict, entry: str, df_filtered) -> pd.Series:
    ...
    for barcode in count_sorted.to_dict():
        ...
        set_result_value(extractor, result_dict, entry.replace(".barcoded", ".") + barcode + ".frequency", frequency_value)
