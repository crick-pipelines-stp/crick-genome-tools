...
class TestIterativeAlignment(unittest.TestCase):
    ...
    def test_iterative_alignment(self):
        ...
        final_consensus_path = os.path.join(
            output_path, sample_id, f"{sample_id}_final_consensus.fasta"
        )
        ...
        final_bam_path = os.path.join(
            output_path, sample_id, f"{sample_id}_final.bam"
        )
        assert os.path.exists(final_bam_path), "Final BAM file not found."

        final_flagstat_path = os.path.join(
            output_path, sample_id, f"{sample_id}_final.flagstat"
        )
        ...
        for i in range(1, 3):
            iteration_dir = os.path.join(
                output_path, sample_id, f"iteration_{i}"
            )
            ...
            consensus_fasta_path = os.path.join(
                iteration_dir, f"{sample_id}_iter_{i}.consensus.fasta"
            )
